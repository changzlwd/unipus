...
###########
#houchaoying add for model name 20260302 s
# Target naming
PRODUCT_NAME := volcano
PRODUCT_DEVICE := volcano
PRODUCT_BRAND := OSCAL
PRODUCT_MODEL := PILOT 8
PRODUCT_MANUFACTURER:= OSCAL
#houchaoying add for model name 20260302 e

# uclogcat caomingyuan add 20250415 s
PRODUCT_PACKAGES += \
     uclogcatserver \
     vendor.unicair.uclogcatclient@4.0-service \
     vendor.unicair.uclogcatclient@4.0-impl \
     vendor.unicair.uclogcatclient@4.0_vendor
#UCLogcat
PRODUCT_PACKAGES += vendor.unicair.uclogcatclient@4.0
# uclogcat caomingyuan add 20250415 e
#----------------------------------------------------------------------
# wlan specific
#----------------------------------------------------------------------
ifeq ($(TARGET_USES_QMAA), true)
...
include device/qcom/wlan/volcano/wlan.mk
endif
else
include device/qcom/wlan/volcano/wlan.mk
endif
...
DEVICE_FRAMEWORK_MANIFEST_FILE := device/qcom/volcano/framework_manifest.xml
...
# Add vintf device manifests for chipsets in volcano QCV family below.
ifeq ($(TARGET_USES_QMAA), true)
...
DEVICE_MANIFEST_VOLCANO_FILES := device/qcom/volcano/manifest_volcano.xml
...
#wangchunyan 20251210 add for compile build s
ifeq ($(TARGET_BUILD_GMS),true)
PRODUCT_PROPERTY_OVERRIDES += \
    ro.vendor.model_gms=true
else
PRODUCT_PROPERTY_OVERRIDES += \
    ro.vendor.model_gms=false
endif

ifeq ($(TARGET_BUILD_REGION),eea)
PRODUCT_PROPERTY_OVERRIDES += \
    ro.vendor.build_region=eea
else ifeq ($(TARGET_BUILD_REGION),ru)
PRODUCT_PROPERTY_OVERRIDES += \
    ro.vendor.build_region=ru
else ifeq ($(TARGET_BUILD_REGION),tr)
PRODUCT_PROPERTY_OVERRIDES += \
    ro.vendor.build_region=tr
else
PRODUCT_PROPERTY_OVERRIDES += \
    ro.vendor.build_region=row
endif
#wangchunyan 20251210 add for compile build e
...
# add  fingerprint by guoyongcan
$(call inherit-product-if-exists, vendor/silead/products/fingerprint.mk)
...
#widevine e
#houchaoying 20260312 add RKP
PRODUCT_PACKAGES += rkp_factory_extraction_tool
#houchaoying 20260312 copy google key
ifeq ($(TARGET_BUILD_REGION),eea)
PRODUCT_COPY_FILES += device/qcom/volcano/keybox/keybox_eea.xml:vendor/bin/keybox.xml
else ifeq ($(TARGET_BUILD_REGION),ru)
PRODUCT_COPY_FILES += device/qcom/volcano/keybox/keybox_ru.xml:vendor/bin/keybox.xml
else
PRODUCT_COPY_FILES += device/qcom/volcano/keybox/keybox_neu.xml:vendor/bin/keybox.xml
endif
###################################################################################
...
###################################################################################
#add zhangyuhan 20260404 for charger font
#PRODUCT_COPY_FILES += device/qcom/volcano/charger_fonts/18x32.png:$(TARGET_COPY_OUT_VENDOR)/etc/res/images/default/charger/font.png
#add by dingyaoyao for charger font 20250111,merge from U812
$(call inherit-product-if-exists, device/qcom/volcano/charger_fonts/*.mk)