...
extern "C" {
int consumerir_start_aging_test(int carrier_freq, int duration_ms);
}
...
class ConsumerIr : public BnConsumerIr {
  ...
  private:
    ...
    ::ndk::ScopedAStatus startAgingTest(int32_t in_carrierFreqHz, int32_t in_durationMs) override;
...
::ndk::ScopedAStatus ConsumerIr::transmit(int32_t in_carrierFreqHz,
    ...
    ALOGE("ConsumerIr::transmit called: carrierFreq=%d, patternSize=%zu", in_carrierFreqHz, in_pattern.size());
...
::ndk::ScopedAStatus ConsumerIr::startAgingTest(int32_t in_carrierFreqHz, int32_t in_durationMs) {
    ALOGE("ConsumerIr::startAgingTest called: carrierFreq=%d, durationMs=%d", in_carrierFreqHz, in_durationMs);

    if (in_carrierFreqHz <= 0 || in_durationMs <= 0) {
        ALOGE("ConsumerIr::startAgingTest: invalid parameters");
        return ::ndk::ScopedAStatus::fromExceptionCode(EX_UNSUPPORTED_OPERATION);
    }

    int ret = consumerir_start_aging_test(in_carrierFreqHz, in_durationMs);
    if (ret != 0) {
        ALOGE("ConsumerIr::startAgingTest failed: %d", ret);
        return ::ndk::ScopedAStatus::fromExceptionCode(EX_UNSUPPORTED_OPERATION);
    }
